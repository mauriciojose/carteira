import { Router } from "express";
import Carteira from "../../controllers/carteira";

class CarteirasRouter {
    controller: Carteira;
    path: string;
    routes: Router;
    
    constructor(routes: Router) {
        this.controller = new Carteira();
        this.path = '/api/carteiras';
        this.routes = routes;

        this.init();
    }
    async init(){
        this.routes.get(this.path + '/', this.controller.getAll);
        this.routes.get(this.path + '/:id', this.controller.get);
        this.routes.get(this.path + '/user/:id', this.controller.getByUserId);
        this.routes.post(this.path + '/', this.controller.create);
        this.routes.put(this.path + '/:id', this.controller.update);
        this.routes.delete(this.path + '/:id', this.controller.delete);
    }
}

export default CarteirasRouter;
